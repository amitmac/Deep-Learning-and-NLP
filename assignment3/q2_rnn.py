...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        l = len(sentence)
        sentence = sentence[:max_length] + [zero_vector] * (max_length - l)
        labels = labels[:max_length] + [zero_label] * (max_length - l)
        mask = [True] * (l if l <= max_length else max_length) + [False] * (max_length - l)
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b_2 = tf.get_variable("b_2", shape=(self.config.n_classes,), initializer=tf.zeros_initializer())
        ### END YOUR CODE

        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ### YOUR CODE HERE (~6-10 lines)
                inputs = tf.reshape(x[:,time_step,:],(-1, self.config.n_features * self.config.embed_size))
                if time_step > 0:
                    tf.get_variable_scope().reuse_variables()
                o_t, h_t = cell(inputs, h)
                o_drop_t = tf.nn.dropout(o_t, self.dropout_placeholder)
                # print "o_drop_t shape", o_drop_t.get_shape().as_list()
                y_t = tf.matmul(o_drop_t, U) + b_2
                # print "y_t shape", y_t.get_shape().as_list()
                # print "input size", Config.n_features * Config.embed_size
                preds.append(y_t)
                h = h_t
                ### END YOUR CODE
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        cross_entropy_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=preds)
        x = tf.boolean_mask(cross_entropy_loss, self.mask_placeholder)
...
if __name__ == "__main__":
    ...
    path = "/workspace/cs224/assignment3/"
    ...
    command_parser.add_argument('-dt', '--data-train', type=argparse.FileType('r'), default=path + "data/tiny.conll", help="Training data")
    command_parser.add_argument('-dd', '--data-dev', type=argparse.FileType('r'), default=path + "data/tiny.conll", help="Dev data")
    command_parser.add_argument('-v', '--vocab', type=argparse.FileType('r'), default=path + "data/vocab.txt", help="Path to vocabulary file")
    command_parser.add_argument('-vv', '--vectors', type=argparse.FileType('r'), default=path + "data/wordVectors.txt", help="Path to word vectors file")
    ...
    command_parser.add_argument('-dt', '--data-train', type=argparse.FileType('r'), default=path + "data/train.conll", help="Training data")
    command_parser.add_argument('-dd', '--data-dev', type=argparse.FileType('r'), default=path + "data/dev.conll", help="Dev data")
    command_parser.add_argument('-v', '--vocab', type=argparse.FileType('r'), default=path + "data/vocab.txt", help="Path to vocabulary file")
    command_parser.add_argument('-vv', '--vectors', type=argparse.FileType('r'), default=path + "data/wordVectors.txt", help="Path to word vectors file")
    command_parser.add_argument('-c', '--cell', choices=["rnn", "gru"], default="rnn", help="Type of RNN cell to use.")
    ...
    command_parser.add_argument('-d', '--data', type=argparse.FileType('r'), default=path + "data/dev.conll", help="Training data")
