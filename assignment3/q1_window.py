...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    windowed_data = []
    start_list = [start] * window_size
    end_list = [end] * window_size

    for sentence, labels in data:
    ### YOUR CODE HERE (5-20 lines)
        l = len(sentence)
        sentence = start_list + sentence + end_list
        for i in range(window_size, window_size + l):
            x = sentence[i-window_size:i] + [sentence[i]] + sentence[(i+1):(i+1+window_size)]
            x = [k for y in x for k in y]
            d = (x, labels[i-window_size])
