...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        W = tf.Variable(tf.zeros(shape=(Config.n_features, Config.n_classes)))
        b = tf.Variable(tf.zeros(shape=(1, Config.n_classes)))
        scores = tf.add(tf.matmul(self.input_placeholder, W), b)
        pred = softmax(scores)
        ### END YOUR CODE
        return pred
